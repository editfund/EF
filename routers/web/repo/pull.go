...
func viewPullFiles(ctx *context.Context, specifiedStartCommit, specifiedEndCommit string, willShowSpecifiedCommitRange, willShowSpecifiedCommit bool) {
	...
	if willShowSpecifiedCommit {
		...
		if prevCommit != nil {
			ctx.Data["PrevCommitLink"] = path.Join(ctx.Repo.RepoLink, "pulls", strconv.FormatInt(issue.ID, 10), "commits", prevCommit.ID.String())
		}
		if nextCommit != nil {
			ctx.Data["NextCommitLink"] = path.Join(ctx.Repo.RepoLink, "pulls", strconv.FormatInt(issue.ID, 10), "commits", nextCommit.ID.String())
		}
